...
def get_rgo_sampling(xk, eta, potential, device, threshold, minimizer=None):
...
def get_samples(y, eta, potential, num_samples, device,threshold=1e-3,minimizer=None):
    # Sampling from potential \prop exp( - f(x) - |x-y|^2/2eta)
    # y = [n,d] outputs [n,num_samples,d]
    n, d = y.shape[0], y.shape[-1]
    yk = y.repeat_interleave(num_samples,dim=0)
    samples, accepted_idx = get_rgo_sampling(yk,eta,potential,device, threshold, minimizer=minimizer)
